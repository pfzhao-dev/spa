...
class Uses : public Relational {
 public:
  Uses(std::shared_ptr<Ref> subject, std::shared_ptr<EntityRef> target);

  Uses(uint32_t stmt_no, std::string var_name);

  Uses(std::string proc_name, std::string var_name);
